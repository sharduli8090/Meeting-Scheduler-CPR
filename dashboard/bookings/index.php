...
foreach ($data as $key => $value) {
    if ($value["email"] === $activeUser && $value["status"] === "true") {
        ...
        <div class="dashboard">
            <div class="restrict">
                <div class="dashboard_header">
                    ...
                    <a href="/dashboard/bookings" class="active">Booked Events</a>
                </div>
            </div>
        </div>
        <div class="restrict">
        ...
        log.append(
            $(`<div class="et_box"></div>`).append(
                $(`<div class="et_row"></div>`).append(
                    $(`<a href="/${meet.data.path}/${meet.data.token}" class="et_name">${meet.data.name}</a>`),
                    $(`<a href="/dashboardbookings/action.delete/?token=${meet.token}" class="et_del"><i class='far fa-trash'></i></a>`),
                ),
                $(`<div class="et_row"></div>`).append(
                    $(`<div class="et_desc"><span>${meet.data.alias} (${meet.data.user})</span><br>Date: <span>${meet.data.date.split("-").reverse().join("-")}</span><br>Time slot: <span>${meet.slot}</span></div>`),
